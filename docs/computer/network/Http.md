...
`http`: 是一个客户端和服务器端请求和应答的标准（基于TCP/IP协议），用于从 WWW 服务器传输资源到本地浏览器的超文本传输协议。
...
- 数字签名：身份验证。签名由发送者发出，是他的凭证，因为是由他的私钥加密，对应的就可以用他的公钥解密。之后便可以拿到对应的摘要（摘要是将传递的信息利用哈希散列生成的）再进行对比原信息查看发送的信息是否正确。但我们需要查看对应的公钥是否就是这个发送者的，此时便可以使用ca认证（证书认证，即数字签名是需要证书认证的）
...
2. 解决
   ...
   3. **proxy代理** 目前常用方式,通过服务器设置代理（代理服务器也需要进行跨域设置，如cors）
...
- **「灵活可扩展」**。HTTP1.0中新增的HEADERS，语法上只规定了基本格式，空格分隔单词，换行分隔字段等。所以只要客户端和服务器端两方商定好新的Header，那么新功能就可以很快被加入进来。
...
- OPTIONS 请求可以探测服务器功能和配置，一般也是用于`客户端查看服务器的性能和特性`。
- 这个方法会请求服务器返回该资源所支持的所有 HTTP 请求方法，该方法会用'*'来代替资源名称，向服务器发送 OPTIONS 请求，可以`测试服务器功能是否正常`。
- 获取允许的头部字段：服务器可以在响应的 Access-Control-Allow-Headers 头部字段中返回支持的头部字段列表，以告知客户端可以使用哪些自定义的头部字段。
- JS 的 XMLHttpRequest对象进行 CORS 跨域资源共享时，对于复杂请求，就是使用 OPTIONS 方法发送`预检请求`，以判断是否有对指定资源的访问权限。

问：谈一谈队头阻塞？

- 首先，http是基于tcp/ip协议上建立的，为了提高效率不用每一次请求都建立tcp连接，便有了长连接（http/1.1默认；http/1.0需手动开启，即connection：keep-alive），一次建立tcp请求，多次复用。
- 但因为请求是逐个去请求的，所以在之前的请求响应回来之前，后续的请求无法进行，这就造成了队头阻塞（因为是前面的请求阻塞导致的）

问：如何解决队头阻塞？

- `管线化`：允许客户端在已发送的请求接收到服务端的响应之前发送下一个请求。  ===>   但是服务器端还是FIFO模式，并未完全解决。（现代浏览器默认已经不适用）
...
- 引入了管线化机制（ pipelining），即在同一个TCP连接里，客户端可以同时发送多个 请求，进一步改进了HTTP协议的效率。
